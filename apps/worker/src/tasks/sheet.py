...
def upload_user_operations(user_id: int):
    ...
    Operation.update(status=OperationStatuses.Processing).where(
        Operation.id.in_([o.id for o in operations])
    ).execute()
    ...
    try:
        ...
        Operation.update(status=OperationStatuses.Error).where(
            Operation.id.in_([o.id for o in operations])
        ).execute()
    ...
    Operation.update(status=OperationStatuses.Uploaded).where(
        Operation.id.in_([o.id for o in operations])
    ).execute()
