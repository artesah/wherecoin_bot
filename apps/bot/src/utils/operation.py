...
async def send_operation_message(bot: Bot, operation: Operation):
    text = _content["messages"]["new_operation"].format(
        type=_content["constants"]["OperationTypes"][
            OperationTypes(operation.type).name
        ],
        source=_content["constants"]["OperationSources"][
    ...
    await bot.send_message(chat_id=chat.id, text=text, reply_markup=reply_markup)
